...
// ===== Economy-aware room overrides =====
roomSnapshot = function(room) {
  return { code: room.code, name: room.name, gameType: room.gameType, hostId: room.hostId, hostUsername: room.hostUsername, isPrivate: room.isPrivate, players: room.players, spectators: room.spectators || [], chat: (room.chat || []).slice(-80), gameState: room.gameState || null, startedAt: room.startedAt || null, status: room.status || 'open', matchMode: room.matchMode || 'custom', countdownEndsAt: room.countdownEndsAt || null, economy: room.economy || null, share: { code: room.code, path: `/join/${room.code}`, deepLink: `warhex://join/${room.code}` } };
};
buildRoom = function({ code, gameType, name, hostSocketId, hostUsername, isPrivate = true, matchMode = 'custom', entryFee = null }) {
  return { code, name: name || `${gameType} room`, gameType: gameType || 'chess', hostId: hostSocketId, hostUsername, isPrivate, players: [{ socketId: hostSocketId, username: hostUsername, ready: matchMode === 'random', connected: true }], spectators: [], chat: [{ sender: 'System', text: `تم إنشاء غرفة ${gameType}`, ts: Date.now() }], gameState: null, startedAt: null, finished: false, status: 'open', matchMode, createdAt: Date.now(), countdownEndsAt: null, economy: entryFee?.amount ? { entryFee: { currency: entryFee.currency || 'coins', amount: Number(entryFee.amount) || 0 }, winnerPayout: Number(entryFee.winnerPayout || 0) || undefined, systemFee: Number(entryFee.systemFee || 0) || undefined, feePercent: Number(entryFee.feePercent || 0) || undefined, status: 'pending' } : null };
};
createRoom = async function(io, socket, { gameType = 'chess', name, isPrivate = true, entryFee = null } = {}) {
  if (!socket.data.username) return; await leaveQueue(io, socket, { silent: true }); const existingCode = socketToRoom.get(socket.id); if (existingCode) leaveRoom(io, socket, { silent: true }); const code = uniqueRoomCode();
  const room = buildRoom({ code, gameType, name, hostSocketId: socket.id, hostUsername: socket.data.username, isPrivate, matchMode: 'custom', entryFee });
  rooms.set(code, room); socket.join(code); socketToRoom.set(socket.id, code); await persistRoom(room); emitRoom(io, room); socket.emit('room:created', roomSnapshot(room)); logger.info('Room created', { code, gameType, host: socket.data.username, entryFee: room.economy?.entryFee || null });
};
finishMatch = async function(room) {
  if (!room.gameState || room.finished) return; room.finished = true; const gs = room.gameState; const winnerUser = gs.winnerUsername || gs.winner; const isDraw = gs.winner === 'draw' || !gs.winner; const allNames = Object.keys(gs.playerColors || {}); const playerList = room.players.length > 0 ? room.players : allNames.map(n => ({ username: n }));
  const elos = {}; for (const p of playerList) { const u = await db.findUser(p.username); elos[p.username] = u?.elo || 1200; }
  const players = []; for (const p of playerList) { const isWinner = p.username === winnerUser || gs.winner === gs.playerColors?.[p.username]; players.push({ username: p.username, color: gs.playerColors?.[p.username] || p.color || null, result: isDraw ? 'draw' : isWinner ? 'win' : 'loss' }); }
  const eloDeltas = {}; if (players.length === 2 && !isDraw) { const [p1, p2] = players; const winner = p1.result === 'win' ? p1 : p2; const loser = p1.result === 'loss' ? p1 : p2; const newWinnerElo = calcElo(elos[winner.username], elos[loser.username], 1); const newLoserElo = calcElo(elos[loser.username], elos[winner.username], 0); eloDeltas[winner.username] = newWinnerElo - elos[winner.username]; eloDeltas[loser.username] = newLoserElo - elos[loser.username]; }
  if (room.economy?.entryFee?.amount && !room.economy?.collectedAt) await db.collectRoomEntryFees(room);
  const matchId = await db.saveMatch({ game_type: room.gameType, room_code: room.code, players, winner: winnerUser, state_snapshot: gs, duration_sec: room.startedAt ? Math.floor((Date.now() - room.startedAt) / 1000) : 0, finished_at: new Date().toISOString(), economy: room.economy || null });
  for (const p of players) await db.updateUserStats(p.username, { win: p.result === 'win', loss: p.result === 'loss', draw: p.result === 'draw', xpGain: p.result === 'win' ? 100 : p.result === 'draw' ? 35 : 10, eloDelta: eloDeltas[p.username] || 0 });
  await db.settleRoomEconomy(room, isDraw ? 'draw' : winnerUser); await persistRoom(room); logger.info('Match saved', { id: matchId, game: room.gameType, winner: winnerUser || 'draw' });
};
Object.assign(module.exports, { roomSnapshot, createRoom, finishMatch });
