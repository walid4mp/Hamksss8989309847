...
// ===== Enhanced ludo sound wrapper =====
(function () {
  const originalApplyLudoAction = applyLudoAction;
  applyLudoAction = function(game, action, actor) {
    const result = originalApplyLudoAction(game, action, actor);
    if (result && typeof SoundFX !== 'undefined') {
      if (action?.kind === 'roll') { SoundFX.dice?.(); SoundFX.vibrate?.(40); }
      if (action?.kind === 'move') SoundFX.move?.();
      if (result.winner) SoundFX.win?.();
    }
    return result;
  };
  if (typeof window !== 'undefined' && window.LudoEngine) {
    window.LudoEngine.applyLudoAction = applyLudoAction;
  }
})();
